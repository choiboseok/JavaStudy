...
public class MultiChatServer {
	// 
	// 접속 클라이언트 관리 배열
	private ArrayList<Client> clientList = new ArrayList<>(); // 사용자 담는 스레드
	...
	public void serverStart() {
		...
		try {
			...
			while(true) {
				...
				Client client = new Client(soc); // 클라이언트 생성
	...
	public class Client extends Thread {
		...
		public Client(Socket soc) {
			this.soc = soc;
			try {
				writer = new PrintWriter(soc.getOutputStream()); // 받는 행위
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		...
		public void sendAll(String msg) {
				for(int i=0; i<clientList.size(); i++) { // 사용자 수 만큼 메세지 전송
					clientList.get(i).writer.println(msg); 
					clientList.get(i).writer.flush(); // writer 후에 비우는 함수
				}
		}
		@Override
		public void run() {
